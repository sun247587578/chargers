    ...
    <el-row>
      ...
      <el-col :span="24">
        <el-table :data="listData" v-loading="fullscreenLoading" element-loading-text="拼命加载中" border style="width: 120%">
          ...
          <!-- <el-table-column prop="labSubmitProjects" label="工号" :formatter="stateFormat" min-width="120"> </el-table-column> -->
          ...
          <el-table-column prop="experienceCompany" label="经历单位" min-width="300"></el-table-column>
          <el-table-column prop="experienceDept" label="经历部门" min-width="300"></el-table-column>
          <el-table-column prop="experiencePost" label="经历职务" min-width="300"></el-table-column>
          <el-table-column prop="remark" label="备注" min-width="300"></el-table-column>
    ...
    <el-dialog title="证件管理" :visible.sync="detailIO">
      <table class="table tableOcc">
        ...
        <tr>
          ...
          <td>结束时间 : </td>
          <td style="text-align:left">
            <el-date-picker v-model="form.endDate" type="date" placeholder="" style="width:100%" @change="sfomartData">
            </el-date-picker>
          </td>
        </tr>
        <tr>
          <td>经历单位 : </td>
          <td colspan="3">
            <el-input v-model="form.experienceCompany"></el-input>
          </td>
        </tr>
        <tr>
          <td>经历部门 : </td>
          <td colspan="3">
            <el-input v-model="form.experienceDept"></el-input>
          </td>
        </tr>
        <tr>
          <td>经历职务 : </td>
          <td colspan="3">
            <el-input v-model="form.experiencePost"></el-input>
          </td>
        </tr>
        <tr>
          <td>备注 :</td>
...
<script>
import { readWorkExperienceList , deleteWorkExperience , updataWorkExperience } from "@/api/hr"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      updataWorkExperience(this.form).then(res => {
        console.log(res)
        if (res.httpCode == 200) {
          this.$message({
            type: 'success',
    ...
    removeData(index) {
      ...
      this.$confirm('确定要执行该操作吗?', '提示', {
        ...
        deleteWorkExperience({ id: this.listData[index].id }).then(res => {
          if (res.httpCode == 200) {
            this.$message({
              type: 'success',
              message: '操作成功!'
    ...
    readData() {
      ...
      readWorkExperienceList(this.search).then(response => {
