<template>
    <div>
        <template v-for="item in routes">
            <router-link v-if="!item.hidden&&item.noDropdown&&item.children.length>0" :to="child.path" >
               ...
               <template slot="title">
                ...
                <template v-for="child in item.children" v-if='!child.hidden'>
                    ...
                    <router-link v-else class="menu-indent" :to="child.path">
                        <el-menu-item :index="item.path+'/'+child.path" :class="{active:$route.matched[0].path===child.path}">
                            {{child.name}}
                        </el-menu-item>
                    </router-link>
                </template>
