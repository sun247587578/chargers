<template>
    <div id="announcement">
                ...
                <el-table :data="AnnouncementList" :fit="true" :resizable="true" v-loading="fullscreenLoading" element-loading-text="拼命加载中" border style="width: 100%">
                    ...
                    <el-table-column fixed="right" label="操作" width="120">
                        <template scope="scope">
                            ...
                            <el-button @click="compile(scope.$index)" type="text" size="small">编辑</el-button>
                            <el-button @click="removeData(AnnouncementList[scope.$index].id)" type="text" size="small">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <!-- <div class="block">
        ...
        <div v-else>
            ...
            <release :snew="2" :sdata="detaildata"  @goBack="goBack"></release>
        </div>
    </div>
</template>

<script>
import { getAnnouncementList , getAnnouncementMyList , deltAnnouncementMyList} from "@/api/oa"
import { release } from "./compontent"
...
export default {
    ...
    methods: {
        ...
        removeData(sid){
            console.log(sid)
            // deltAnnouncementMyList()
            this.$confirm('确定要执行吗?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                deltAnnouncementMyList({ id: sid}).then(res => {
                    if (res.httpCode == 200) {
                        this.$message({
                            type: 'success',
                            message: '操作成功!'
