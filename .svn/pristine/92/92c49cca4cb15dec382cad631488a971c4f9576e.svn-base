    ...
    <el-row>
      ...
      <el-col :span="24">
        <el-table :data="listData" v-loading="fullscreenLoading" element-loading-text="拼命加载中" border style="width: 120%">
          ...
          <el-table-column prop="dept" label="兼职部门" min-width="180"></el-table-column>
          <el-table-column prop="post" label="兼职岗位" min-width="180"></el-table-column>
          <el-table-column prop="rewardDate" label="开始日期" min-width="180" :formatter="stateFormat1"></el-table-column>
          <el-table-column prop="endDate" label="结束日期" min-width="180"></el-table-column>
          <!-- <el-table-column prop="rewardAmount" label="奖励金额" min-width="150"></el-table-column> -->
          <el-table-column prop="stopFlg" label="停止状态" min-width="150" :formatter="stateFormat"></el-table-column>
      ...
      <table class="table tableOcc">
        ...
        <tr>
          ...
          <td style="text-align:left" colspan="3">
            <el-switch v-model="form.stopFlg" on-text="" off-text="" on-value="0" off-value="1">
            </el-switch>
          </td>
        </tr>
        <tr>
...
<script>
import { readPartTimeJobList, deletePartTimeJob, updataPartTimeJob } from "@/api/hr"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      updataPartTimeJob(this.form).then(res => {
        console.log(res)
        if (res.httpCode == 200) {
          this.$message({
            type: 'success',
    ...
    removeData(index) {
      ...
      this.$confirm('确定要执行该操作吗?', '提示', {
        ...
        deletePartTimeJob({ id: this.listData[index].id }).then(res => {
          if (res.httpCode == 200) {
            this.$message({
              type: 'success',
              message: '操作成功!'
    ...
    readData() {
      ...
      readPartTimeJobList(this.search).then(response => {
