<template>
  <div>
    <el-row>
      ...
      <el-col :span="24">
        <el-table :data="listData" v-loading="fullscreenLoading" element-loading-text="拼命加载中" border style="width: 120%">
          ...
          <el-table-column prop="punishType" label="惩罚种类" min-width="120"></el-table-column>
          <el-table-column prop="punishStyle" label="惩罚方式" min-width="300"></el-table-column>
          <el-table-column prop="punishDate" label="惩罚日期" min-width="300" :formatter="stateFormat1"></el-table-column>
          <el-table-column prop="punishAuthorizer" label="惩罚批准人" min-width="300"></el-table-column>
          <el-table-column prop="punishAmount" label="惩罚金额" min-width="150"></el-table-column>
          <el-table-column prop="receiptNumber" label="单据编号" min-width="300"></el-table-column>
          <el-table-column prop="punishReason" label="惩罚原因" min-width="300"></el-table-column>
    ...
    <el-dialog title="惩罚" :visible.sync="detailIO">
      <table class="table tableOcc">
        ...
        <tr>
          <td>惩罚种类 : </td>
          <td>
            <el-input v-model="form.punishType"></el-input>
          </td>
          <td>惩罚方式 : </td>
          <td>
            <el-input v-model="form.punishStyle"></el-input>
          </td>
        </tr>
        <tr>
          <td>惩罚日期 : </td>
          <td>
            <el-date-picker v-model="form.punishDate" type="date" placeholder="" style="width:100%" @change="sfomartData">
            </el-date-picker>
          </td>
          <td>批准人 : </td>
          <td>
            <el-input v-model="form.punishAuthorizer"></el-input>
          </td>
        </tr>
        <tr>
          <td>惩罚金额 : </td>
          <td>
            <el-input v-model="form.punishAmount"></el-input>
          </td>
          <td>单据编号 : </td>
          <td style="text-align:left">
        ...
        <tr>
          <td>惩罚原因</td>
          <td colspan="5">
            <el-input type="textarea" v-model="form.punishReason"></el-input>
          </td>
        </tr>
        <tr>
          <td>备注</td>
...
<script>
import { readPunishmentList , deletePunishment , updataPunishment } from "@/api/hr"
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      updataPunishment(this.form).then(res => {
        console.log(res)
        if (res.httpCode == 200) {
          this.$message({
            type: 'success',
    ...
    removeData(index) {
      ...
      this.$confirm('确定要执行该操作吗?', '提示', {
        ...
        deletePunishment({ id: this.listData[index].id }).then(res => {
          if (res.httpCode == 200) {
            this.$message({
              type: 'success',
              message: '操作成功!'
    ...
    readData() {
      ...
      readPunishmentList(this.search).then(response => {
